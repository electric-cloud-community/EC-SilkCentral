...
// SilkCentralConfigListLoader.java --
//
// SilkCentralConfigListLoader.java is part of ElectricCommander.
//
// Copyright (c) 2005-2011 Electric Cloud, Inc.
// All rights reserved.
//

package ecplugins.silkCentral.client;
...
public class SilkCentralConfigListLoader
    extends Loader
{

    //~ Instance fields --------------------------------------------------------

    private final SilkCentralConfigList m_configList;
    private final CgiRequestProxy       m_cgiRequestProxy;
    private String                      m_editorName;

    //~ Constructors -----------------------------------------------------------

    public SilkCentralConfigListLoader(
            SilkCentralConfigList configList,
            Component             component,
    ...
    private void loadConfigs(Map<String, String> cgiParams)
    {
        ...
        try {
                    ...
                    new RequestCallback() {
                                ...
                                Throwable exception)
                        {
                            ((HasErrorPanel) m_component).addErrorMessage(
                                "Error loading SilkCentral configuration list: ",
                                exception);
                        }
            ...
            if (m_component instanceof HasErrorPanel) {
                ((HasErrorPanel) m_component).addErrorMessage(
                    "Error loading SilkCentral configuration list: ", e);
            }
            else {
                m_component.getLog()
                           .error(e);
    ...
    private void loadEditors()
    {
        ...
        request.setPropertyName("/plugins/EC-SilkCentral/project/ui_forms/"
                + m_editorName);
        request.setExpand(false);
        request.setCallback(new EditorLoaderCallback("silkCentralcfg"));
        ...
        implements PropertyCallback
    {
        ...
        @Override public void handleResponse(Property response)
        {
            ...
            String errorMsg = "Editor '" + m_editorName
                    + "' not found for SilkCentral plugin '" + m_configPlugin
                    + "'";
